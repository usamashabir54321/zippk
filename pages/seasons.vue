				...
				<div class="row justify-content-center">
					<div class="col-lg-8">
						...
						<div class="section-title title-style-three text-center mb-40">
							<h2 class="title">OUR ALL SEASONS</h2>
						</div>
					</div>
				</div>
...
<script>
	...
	export default {
		...
		methods: {
			async getData() {
				const data = axios.get(`${this.baseURL}api/webOnlyGet/seasons`);
			...
			async loadMore() {
				...
				const data = axios.get(`${this.baseURL}api/webGetById/loadSeasons/${lasCardId}`);
				const result = await data;
